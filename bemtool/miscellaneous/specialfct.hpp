...
// ATTENTION: ROUTINE BUGEE POUR LE MOMENT....
  ...
  inline C3 Grad_SphHarmo(const int& n, const int& m, const R3& y){
    if(n==0){return C3_(0.,0.,0.);}  
    R3 x = y; normalize(x);
    Real theta      = Sph::Theta(x);
    Real phi        = Sph::Phi(x);
    R3   e_phi      = R3_(-sin(phi),cos(phi),0.);
    R3   e_theta    = vprod(x,e_phi);
    Real cos_theta  = cos(theta);
    Real sin_theta  = sin(theta);


    if(sin_theta <1e-8){return C3_(0.,0.,0.);}
    Real coef       = sqrt( (2*n+1)*( n-abs(m) )/(2*n-1 ) );
    Cplx du_dphi, du_dtheta;
    du_dtheta = (n*cos_theta*SphHarmo(n,m,x)-coef*SphHarmo(n-1,m,x) )/sin_theta;
    du_dphi   = iu*m*SphHarmo(n,m,x)/sin_theta;
    return du_dphi*e_phi + du_dtheta*e_theta;
  }
